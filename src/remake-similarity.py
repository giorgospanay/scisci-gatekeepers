...
This version:
  ...
  - Automatically resumes if temporary chunks already exist (no manual input)
...
# ===== STEP 1: Handle existing chunks (auto-resume logic) =====
existing_chunks = sorted(glob.glob(os.path.join(tmp_dir, "author_edges_chunk*.csv")))
if existing_chunks:
	chunk_idx = max(
		int(os.path.basename(f).split("chunk")[-1].split(".")[0])
		for f in existing_chunks
	) + 1
	print(f"⚠️  Found {len(existing_chunks)} existing chunk files.")
	print(f"Will resume writing from chunk index {chunk_idx:04d}.")
else:
	chunk_idx = 0
	print("No existing chunk files found — starting from scratch.")

print("-" * 90)

# ===== STEP 2: Build paper → authors mapping =====
...
# ===== STEP 3: Stream paper–paper edges in chunks =====
...
def flush_chunk():
	...
	if os.path.exists(tmp_path):
		print(f"  ⏭️  Skipping existing {tmp_path}")
	else:
