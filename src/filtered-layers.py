...
obj_path = f"{out_workspace_path}/obj"          # final outputs on SLATE
scratch_obj_path = f"{out_scratch_path}/obj"   # intermediates on SCRATCH
...
nprobe = 50      # clusters to probe per query
use_gpu = True   # set False if running on CPU partition
...
# --- Step 3: Build Author Similarity Layer with Approximate FAISS ---
...
print(f"Building FAISS IVF index for {len(paper_ids_filtered)} papers")
...
print(f"Querying nearest neighbors (top_k={top_k}, nlist={nlist}, nprobe={nprobe})...")

# Run ANN search in batches with progress + ETA
